...
### 5. Training
`Trainer.java` shows the network thousands of labeled audio clips. For each one:
...
└── src/
    └── main/
        ...
        │   ├── training/
        │   │   └── Trainer.java             training loop — loads features, calls forward + backward
...
**`backward(Matrix resultMatrix, int label)`** — takes the output of forward and the correct emotion index. Computes gradients for all four weight/bias matrices and updates them. Gradient matrices are named as their twin weight matrix + `2` (e.g. `weightsHiddenOutput2` is the gradient for `weightsHiddenOutput`).
...
### Trainer.java
Runs the training loop. For each epoch, loops through all training files, extracts MFCC features, calls `forward()` then `backward()` on the network. Prints the average loss every 50 epochs so you can watch it decrease during training.
...
### Main.java
Entry point. Calls everything in order. At the end, plays a sample `.wav` file through the system speakers using `javax.sound.sampled` (built into Java — no extra library needed) and prints the network's predicted emotion.
...
### Clone and build
```bash
git clone https://github.com/your-repo/speech_emotion_recognition.git
